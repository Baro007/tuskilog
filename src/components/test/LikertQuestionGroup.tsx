...
interface LikertQuestionGroupProps {
  questions: LikertQuestionType[]
...
export function LikertQuestionGroup({
  ...
  const [currentAnswers, setCurrentAnswers] = useState<Record<string, number | null>>(values)

  useEffect(() => {
    setCurrentAnswers(values)
  }, [values])

  const handleQuestionChange = (questionIndex: number, value: number) => {
