...
export default function LifestyleTestPage() {
  ...
  const lifestyleModule = getModuleById('lifestyle')
  const questions = lifestyleModule?.questions as LikertQuestion[]
  ...
  useEffect(() => {
    ...
    questions?.forEach((_, index) => {
      const answer = testStore.getAnswer(`lifestyle-${index + 1}`)
      if (answer !== null) {
  ...
  const handleAnswerChange = (questionIndex: number, value: number) => {
    ...
    const testStore = TestStore.getInstance()
    testStore.updateAnswer(`lifestyle-${questionIndex + 1}`, value)
  }
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      if (nextModuleId === 'mbti-ei') router.push(ROUTES.TEST.MBTI_EI)
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-8">
        <div className="flex items-center justify-between text-sm text-gray-600 mb-2">
          <span>Modül 7 / 12</span>
          <span>{lifestyleModule?.title}</span>
        </div>
      </div>

      <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-8 mb-8">
        <div className="flex items-center mb-4">
          <div className="w-12 h-12 bg-primary-600 text-white rounded-full flex items-center justify-center font-bold text-lg mr-4">7</div>
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{lifestyleModule?.title}</h1>
            <p className="text-gray-600 mt-1">{lifestyleModule?.description}</p>
          </div>
        </div>
        <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
          <p className="text-emerald-800 text-sm"><strong>Yönerge:</strong> Yaşam tarzı tercihleriniz ve günlük hayat yaklaşımınızı değerlendirin.</p>
        </div>
      </div>
      ...
      <div className="bg-lime-50 border border-lime-200 rounded-lg p-4 mb-8">
        <h3 className="font-semibold text-lime-900 mb-2">💡 İpucu</h3>
        <p className="text-lime-800 text-sm">Yaşam tarzı tercihleri iş-hayat dengenizi ve mesleki memnuniyetinizi doğrudan etkiler.</p>
      </div>

      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-8">
        <h3 className="font-semibold text-blue-900 mb-2">🎯 Önemli Not</h3>
        <p className="text-blue-800 text-sm">
          Yaşam tarzı modülünden sonra <strong>MBTI kişilik tipi</strong> değerlendirmesine geçeceğiz. 
          Bu bölüm 4 farklı boyutta 16 soru içerir.
        </p>
      </div>
      ...
      {isLoading && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 flex items-center space-x-3">
            ...
            <span className="text-gray-700">MBTI değerlendirmesi yükleniyor...</span>
          </div>
        </div>
      )}
    </div>
